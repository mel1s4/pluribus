...
import { useOfferEditForm } from '../../composables/useOfferEditForm'
...
const offerId = computed(() => {
  const raw = route.params.offerId
  return typeof raw === 'string' ? raw : ''
})
...
const {
  ...
  goToStep,
  submit,
} = useOfferEditForm(placeId, offerId)
...
<template>
  <section class="offer-create">
    <div class="offer-create__head">
      ...
      <Title tag="h1">{{ t('myPlaces.editOfferPageTitle') }}</Title>
      <p class="offer-create__intro">Edit your offer details below.</p>
    ...
    <div class="offer-create__layout">
      <div class="offer-create__main">
        ...
        <form v-if="isLoaded" class="offer-create__panel" @submit.prevent="onSubmit">
          ...
          <footer class="offer-create__actions">
            ...
            <Button v-else type="submit" variant="primary" :disabled="isSubmitting">
              {{ isSubmitting ? t('myPlaces.offerSubmitting') : 'Save Changes' }}
            </Button>
          </footer>
        </form>
      </div>
