...
class UserAdminController extends Controller
{
    public function index(Request $request): AnonymousResourceCollection
    {
        ...
        if ($search === '' && ! $actor->can('users.view')) {
            abort(403);
        }
    ...
    {
        $actor = $request->user();
        if (! $actor instanceof User || ! $actor->can('users.view')) {
            abort(403);
        }
    ...
    public function update(UpdateUserRequest $request, User $user): JsonResponse
    {
        ...
        return DB::transaction(function () use ($validated, $user, $actor, $request): JsonResponse {
            $data = [
                'name' => $validated['name'],
                'email' => $validated['email'],
            ];
            ...
            if (array_key_exists('voting_id', $validated)) {
                $newId = $validated['voting_id'];
                $oldId = $user->voting_id;
                if ($newId !== $oldId) {
